...
class NetworkCaptureApp:
    def __init__(self, root):
        ...
        self.log_area = scrolledtext.ScrolledText(
            root, width=100, height=25, wrap=tk.WORD
        )
        self.log_area.grid(column=0, row=1, padx=15, pady=15)
        ...
        ttk.Label(root, text="Real-time Statistics:").grid(
            column=0, row=2, padx=15, pady=5, sticky=tk.W
        )
        self.statistics_label = ttk.Label(root, text="")
        self.statistics_label.grid(column=0, row=3, padx=15, pady=5, sticky=tk.W)
    ...
    def packet_handler(self, pkt):
        if self.capture_active:
            ...
            log_message = f"[{packet_type}] {str(pkt)}"
